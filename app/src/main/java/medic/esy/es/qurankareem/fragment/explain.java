...
import medic.esy.es.qurankareem.adapter.ExplainationAdapter;
...
public class explain extends AppCompatActivity {
    ...
    listItems  =new ArrayList<>();
        recyclerView=(RecyclerView)findViewById(R.id.explainationRecycle);
        recyclerView.setHasFixedSize(true);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));

        for (int i=0;i<nameOfSurat.length;i++){
            suratNameExplaination modelExplain =new suratNameExplaination(
                    nameOfSurat[i]
            );

            listItems.add(modelExplain);
            adapter=new ExplainationAdapter(listItems,this);
